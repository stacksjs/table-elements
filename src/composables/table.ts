...
const readableLastColumn = computed(() => lastColumn.value[0]?.includes(':') ? lastColumn.value[0].split(':')[1].trim() : lastColumn.value[0])
...
function goToPrevPage() {
  if (table.currentPage === 1 || currentPage.value === undefined || table === undefined)
    return
...
function goToNextPage() {
  ...
  if (table.currentPage === totalPages.value || currentPage.value === undefined || table === undefined)
    return
...
function goToPage(page: number) {
  ...
  if (table.currentPage === page || currentPage.value === undefined || table === undefined)
    return
